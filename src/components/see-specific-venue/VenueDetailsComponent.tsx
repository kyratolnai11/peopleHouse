...
type VenueScreenRouteProp = RouteProp<RootStackParamList, "Venues">;

type VenueDetailsRouteProps = {
  route: VenueScreenRouteProp;
};
...
const VenueDetailsScreen: React.FC<VenueDetailsRouteProps> = ({ route }) => {
  ...
  if (!venueId) {
    ...
    return <Text>Details for Venue ID not available. Error with venueId</Text>;
  }
    ...
    const getEventsData = async () => {
      ...
      if (eventsFromDB) {
        if (eventsFromDB.items) {
          ...
          const eventItems: Event[] = eventsFromDB.items.filter(
            (item) => item !== null
          ) as Event[];
      ...
      <ScrollView>
        <View style={sharedStyles.mainContainer}>
          ...
          <View style={venueDetailsStyles.firstSection}>
            ...
            <Text style={venueDetailsStyles.firstSectionText}>
          ...
          <View style={venueDetailsStyles.purpleSection}>
            <Text style={venueDetailsStyles.descriptionText}>
              {venue?.description}
            </Text>
            ...
            <View style={venueDetailsStyles.rowSection}>
              <View style={venueDetailsStyles.openingHoursSection}>
                <Text style={venueDetailsStyles.sectionTitle}>
                  Opening Hours
                </Text>
                <Text style={venueDetailsStyles.sectionText}>
                  {venue?.openingHours}
                </Text>
              </View>
              <View>
          ...
          {/* Third Section */}
          <View>
            <Text style={venueDetailsStyles.thirdSectionTitle}>
              Upcoming Events
            </Text>
